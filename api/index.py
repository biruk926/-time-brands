...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "timebrand-dev-secret")
...
def read_blob(filename):
    ...
    if resp.status_code != 200:
        app.logger.error(f"Storage read error for {filename}: HTTP {resp.status_code}")
...
def write_blob(filename, data):
    ...
    if resp.status_code not in (200, 201, 204):
        app.logger.error(f"Storage write failed for {filename}: HTTP {resp.status_code}: {resp.text[:200]}")
...
def create_order():
    ...
    total = round(total, 2)
    orders = load_orders()
    # Robust ID generation: max + 1, but also avoid collision with existing ids
...
def update_order(order_id):
    ...
    data = request.get_json(silent=True) or {}
    # Only allow limited admin updates, not arbitrary status changes (use dedicated endpoints)
    for field in ["userName", "email", "phone", "shippingAddress", "paymentMethod", "paymentReference"]:
        if field in data:
            order[field] = str(data[field]).strip()
    if "items" in data:
        # For simplicity, recalc total from items
        products = load_products()
        ...
        for item in data["items"]:
            try:
                ...
                return _error("Invalid item format", 400)
            product = product_map.get(pid)
            if not product:
                return _error(f"Product #{pid} not found", 400)
            unit_price = float(product["price"])
...
# ---------------------------------------------------------------------------
# Routes: dedicated order status actions
# ---------------------------------------------------------------------------
...
def _change_order_status(order_id, new_status, reason=None):
    ...
    if message_text:
        ...
        if message is None:
            # Message already exists (duplicate action)
            message = next(
